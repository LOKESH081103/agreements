...
def _combine_stats_masked(df: pd.DataFrame, agg_func: str, mask: pd.Series) -> float:
    """Same combine rule as `_pivot_agg_value` (sum of sums, sum/count for
    mean, etc.), but takes an already-built boolean mask directly instead of
    building one from row/col prefixes -- used where the mask needs a field
    deliberately left OUT (e.g. computing a normalize-field denominator)."""
    subset = df.loc[mask]
    if subset.empty:
        return 0.0
    if agg_func == "sum":
...
def build_field_efficiency_pivot(
    ...
    def _build_mask(fields: list[str], prefix: tuple[Any, ...], skip_field: str | None) -> pd.Series:
        mask = pd.Series(True, index=source_df.index)
        for f, v in zip(fields, prefix):
            if f == skip_field:
                continue
            mask &= source_df[f] == v
        return mask
    ...
    def _eff_value(row_prefix: tuple[Any, ...], col_prefix: tuple[Any, ...]) -> float:
        ...
        field_normalize from BOTH masks for the denominator -- wherever it
        happens to live, row side or column side -- is what makes this one
        formula correct for every case: leaf cell, row-group subtotal,
        Total column, and the Total/Total corner alike."""
        numer_mask = _build_mask(field_row_cols, row_prefix, None) & _build_mask(field_col_cols, col_prefix, None)
        numerator = _combine_stats_masked(source_df, agg_func, numer_mask)
        denom_mask = (
            _build_mask(field_row_cols, row_prefix, field_normalize)
            & _build_mask(field_col_cols, col_prefix, field_normalize)
        )
        denominator = _combine_stats_masked(source_df, agg_func, denom_mask)
...
def _render_styled_efficiency_matrix(
    ...
    block's own Total column, plus (when the Rows selection has 2+ fields)
    each subtotal/group row -- exactly the same on-screen treatment as the
    Summary Matrix -- formats as %, falls back to a plain unstyled table if
    styling fails for any reason rather than crashing the page."""
    try:
        def _apply_total_styles(df: pd.DataFrame) -> pd.DataFrame:
            ...
            for i, label in enumerate(row_labels):
                is_group = bool(group_row_flags[i]) if group_row_flags is not None and i < len(group_row_flags) else False
                if label == last_row_label:
                    styles.iloc[i, :] = "background-color:#dbe5f1; font-weight:bold"
                elif is_group:
                    styles.iloc[i, :] = "background-color:#dbe5f1; font-weight:bold"
            for j, is_total in enumerate(total_col_flags):
...
def compute_flow_retention_table(display_df: pd.DataFrame) -> pd.DataFrame:
    """
    Alternate efficiency metric, only for Summary Matrices whose row groups
    break down into Flow / Normalised / Stabilized / (optional) Roll Back
    sub-rows (e.g. an opening-slab table). For every group and every column:

        100 - (Flow / (Flow + Normalised + Stabilized + RollBack) * 100)

    i.e. the % of that group's total that is NOT Flow. A missing sub-row
    (e.g. no Roll Back for that slab) counts as 0. Groups with no Flow
    sub-row at all are skipped (this metric doesn't apply to them). The
    trailing 'Grand Total' row is skipped too.
    """
    aliases = {
        "flow": {"flow"},
    ...
    def match_key(stripped_lower: str) -> str | None:

...
def render_pivot_ratio_calculator(display_df: pd.DataFrame, key_suffix: str) -> None:
    ...
    ('which value / sum of which values'). Result = Numerator / Denominator.
    """
    st.divider()
    st.markdown("#### ➗ Efficiency / Ratio Calculator")
    st.caption(
        "Pick Row(s) × Column(s) from the Summary Matrix above to sum for the Numerator, "
        "then the same for the Denominator. Result = Numerator ÷ Denominator. "
    ...
    if not (num_rows_sel and num_cols_sel and den_rows_sel and den_cols_sel):

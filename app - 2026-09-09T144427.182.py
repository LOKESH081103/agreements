...
def dataframe_to_formatted_excel_bytes(
    ...
    # it already was.
    use_row_hierarchy_style = (table_style == "matrix") and (not index_is_multi) and (n_index_cols == 1)
    ...
    for r in range(n_rows):
        ...
        for c in range(n_index_cols):
            ...
            elif table_style == "matrix":
                level_color = (
                    _XL_LEVEL_FONT_COLORS[depth % len(_XL_LEVEL_FONT_COLORS)]
                    if (use_row_hierarchy_style and not is_total_row)
                    else None
                )

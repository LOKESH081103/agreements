...
def add_dpd_buckets_to_excel_pivot(display_df: pd.DataFrame, slab_level: int) -> pd.DataFrame:
    """
    ...
    '90+' = sum of '90+' alone -- since this is IDENTICAL to the existing raw
            '90+' slab column, it is skipped rather than added as a confusing
            duplicate column with the same label.
    ...
    for bucket_name, slabs in bucket_defs.items():
        if bucket_name in existing_leaf_slabs and slabs == {bucket_name}:
            continue  # identical to an existing raw slab column -- skip duplicate
...
def build_pivot_table(
    ...
    total_row_index = pd.Index([("Total",) + ("",) * (pivot.index.nlevels - 1)]) if isinstance(pivot.index, pd.MultiIndex) else pd.Index(["Total"])
                ...
                if pivot_col_cols:
                    ...
                    if add_dpd_buckets:
                        ...
                        try:
                            ...
                            display_pivot_df = add_dpd_buckets_to_excel_pivot(display_pivot_df, slab_level)
                            st.markdown("##### ➕ Summary Matrix with Cumulative DPD Buckets")

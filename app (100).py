...
def build_excel_style_pivot(
    ...
    if col_fields:
        ...
        col_entries.append({"label": "Grand Total", "prefix": (), "is_total": True, "is_grand_total": True})
    ...
    for re_ in row_entries:
        ...
        row_vals = [
            _pivot_agg_value(work_df, value_col, agg_func, row_fields, re_["prefix"], col_fields, ce["prefix"]) / divisor
            for ce in col_entries
        ]
...
def render_excel_style_pivot_table(
    ...
    display_df, group_row_flags, total_col_flags = build_excel_style_pivot(
        source_df, row_cols, col_cols, value_col, agg_func, column_kind_map, in_crores
    )
...
# --------------------------------------------------------------------------- #
# Stage Efficiency Table: each cell as a % of the sibling Columns-groups'
# total, holding every deeper Columns level (and the Row) fixed.
#
#   e.g. STAGE_1, EAST, July efficiency =
#        value(STAGE_1, EAST, July) / [ value(STAGE_1, EAST, July)
#                                      + value(STAGE_2, EAST, July)
#                                      + value(STAGE_3, EAST, July) ]
#
# i.e. "what share of this July, for this zone, does STAGE_1 represent
# out of STAGE_1 + STAGE_2 + STAGE_3 combined?" — works the same way for
# every zone and every one of however many months/periods are in the data.
# --------------------------------------------------------------------------- #
@st.cache_data(ttl=300, show_spinner=False)
def build_stage_efficiency_pivot(
    source_df: pd.DataFrame,
    row_cols: list[str],
    col_cols: list[str],
    value_col: str,
    ...
    """
    Returns (display_df, group_row_flags, grand_total_col_flags). Every cell
    is a percentage (already ×100) EXCEPT the final "Grand Total" column,
    which is kept as the raw aggregated value (there's nothing meaningful to
    divide it by) — `grand_total_col_flags` marks that column positionally
    so the renderer can format it differently.
    """
    label_map: dict[tuple[str, Any], str] = {}
    work_df, row_fields = _expand_date_fields(source_df, row_cols, column_kind_map, label_map)
    work_df, col_fields = _expand_date_fields(work_df, col_cols, column_kind_map, label_map)
    ...
    # Group every column entry (leaf AND its own subtotal alike) by its
    # "trailing key" — every Columns level except the first — so entries
    # that differ only in the first Columns field's value (the siblings
    # being compared, e.g. STAGE_1 vs STAGE_2 vs STAGE_3) land together.
    trailing_groups: dict[tuple[Any, ...], list[int]] = {}
    for idx, ce in enumerate(col_entries):
        if ce.get("is_grand_total"):
            continue
        trailing_groups.setdefault(ce["prefix"][1:], []).append(idx)
    ...
    for ce in col_entries:
        ...
        col_tuples.append(tup)
    columns_index = pd.MultiIndex.from_tuples(col_tuples) if col_depth > 1 else pd.Index([t[0] for t in col_tuples])
    grand_total_col_flags = [bool(ce.get("is_grand_total")) for ce in col_entries]
    ...
    for re_ in row_entries:
        ...
        raw_vals = [
            _pivot_agg_value(work_df, value_col, agg_func, row_fields, re_["prefix"], col_fields, ce["prefix"])
            for ce in col_entries
        ]
        ...
        for idx, ce in enumerate(col_entries):
            if ce.get("is_grand_total"):
                row_vals.append(raw_vals[idx])
                continue
            group_idx = trailing_groups[ce["prefix"][1:]]
            denom = sum(raw_vals[i] for i in group_idx)
            row_vals.append((raw_vals[idx] / denom * 100.0) if denom else 0.0)
        data_rows.append(row_vals)
    ...
    return display_df, group_row_flags, grand_total_col_flags


def render_stage_efficiency_table(
    source_df: pd.DataFrame,
    row_cols: list[str],
    col_cols: list[str],
    value_col: str,
    ...
    """Build the Stage Efficiency table, render it (styled where possible), return it for CSV export."""
    display_df, group_row_flags, grand_total_col_flags = build_stage_efficiency_pivot(
        source_df, row_cols, col_cols, value_col, agg_func, column_kind_map
    )

    top_field = col_cols[0]
    st.markdown(f"**{agg_func.title()} of {value_col} — % of `{top_field}` group total**")
    st.caption(
        f"Each cell = its value ÷ the sum across every `{top_field}` for that same "
        "row + column-group combination. E.g. Stage 1 / East / July = "
        "value(Stage 1, East, July) ÷ [value(Stage 1, East, July) + value(Stage 2, East, July) + "
        "value(Stage 3, East, July)] — computed the same way for every row and every period in the data. "
        "The **Grand Total** column is left as the raw total (nothing to take a % of)."
    )
    ...
    try:
        ...
        row_labels_for_style = list(display_df.index)
        pct_cols = [c for c, is_gt in zip(display_df.columns, grand_total_col_flags) if not is_gt]
        gt_cols = [c for c, is_gt in zip(display_df.columns, grand_total_col_flags) if is_gt]

        styler = display_df.style.apply(_apply_styles, axis=None)
        if pct_cols:
            styler = styler.format("{:,.2f}%", subset=pct_cols)
        if gt_cols:
            styler = styler.format("{:,.2f}", subset=gt_cols)
        st.dataframe(styler, use_container_width=True)
    except Exception:
        st.dataframe(display_df, use_container_width=True)
...
# --------------------------------------------------------------------------- #
# Post-pivot Efficiency / Ratio Calculator
# --------------------------------------------------------------------------- #
def _flatten_pivot_column_label(col: Any) -> str:
    """Turn a (possibly MultiIndex) pivot column into one readable string,
    e.g. ('STAGE_1', 'Jun-26', '01-Jun-26') -> 'STAGE_1 | Jun-26 | 01-Jun-26',
    and ('STAGE_1 Total', '', '') -> 'STAGE_1 Total'."""
...
def build_pivot_selection_options(display_df: pd.DataFrame) -> tuple[list[str], list[str], dict[str, Any]]:
    """
    Build the Row and Column pick-lists for the ratio calculator directly
    from the already-built Excel-style pivot (`display_df`), plus a
    display-label -> actual-column lookup (columns need this because a
    MultiIndex column's real key is a tuple, not the readable string shown
    in the multiselect).
    """
    row_options = list(dict.fromkeys(display_df.index.tolist()))

    col_display_to_actual: dict[str, Any] = {}
    for c in display_df.columns:
        disp = _flatten_pivot_column_label(c)
        base_disp, suffix = disp, 2
        while disp in col_display_to_actual and col_display_to_actual[disp] != c:
            disp = f"{base_disp} ({suffix})"
            suffix += 1
        col_display_to_actual[disp] = c
    col_options = list(col_display_to_actual.keys())

    return row_options, col_options, col_display_to_actual


def render_pivot_ratio_calculator(display_df: pd.DataFrame, key_suffix: str) -> None:
    """
    Post-pivot 'efficiency calculator': the Numerator and Denominator are
    each built by picking Row(s) x Column(s) from the finished pivot and
    summing every matching cell — so a selection can be a single cell
    ('which value / which value') or several cells summed together
    ('which value / sum of which values'). Each calculated ratio can be
    added to a running results table below, so you can build up several
    ratios (e.g. one per month) and see/download them all together.
    """
    st.divider()
    st.markdown("#### ➗ Efficiency / Ratio Calculator")
    st.caption(
        "Pick Row(s) × Column(s) from the Summary Matrix above to sum for the Numerator, "
        "then the same for the Denominator. Click **Add to Results Table** to save it, and "
        "repeat for as many ratios as you need — they'll all appear in the table below. "
        "⚠️ If you select both a subtotal/Grand Total and its own detail rows or columns, "
        "you'll double-count — pick one or the other."
    )

    row_options, col_options, col_display_to_actual = build_pivot_selection_options(display_df)
    table_key = f"ratio_rows_{key_suffix}"
    st.session_state.setdefault(table_key, [])

    label_input = st.text_input(
        "Label for this ratio (optional, e.g. 'March Efficiency')", key=f"ratio_label_{key_suffix}"
    )

    rc1, rc2 = st.columns(2)
    with rc1:
        st.markdown("**Numerator**")
        num_rows_sel = st.multiselect("Row(s)", options=row_options, key=f"ratio_num_rows_{key_suffix}")
        num_cols_sel = st.multiselect("Column(s)", options=col_options, key=f"ratio_num_cols_{key_suffix}")
    with rc2:
        st.markdown("**Denominator**")
        den_rows_sel = st.multiselect("Row(s)", options=row_options, key=f"ratio_den_rows_{key_suffix}")
        den_cols_sel = st.multiselect("Column(s)", options=col_options, key=f"ratio_den_cols_{key_suffix}")

    ready = bool(num_rows_sel and num_cols_sel and den_rows_sel and den_cols_sel)
    numerator_value = denominator_value = None

    if ready:
        try:
            num_cols_actual = [col_display_to_actual[c] for c in num_cols_sel]
            den_cols_actual = [col_display_to_actual[c] for c in den_cols_sel]
            numerator_value = float(display_df.loc[num_rows_sel, num_cols_actual].to_numpy().sum())
            denominator_value = float(display_df.loc[den_rows_sel, den_cols_actual].to_numpy().sum())
        except Exception as exc:  # noqa: BLE001
            st.error(f"❌ Couldn't compute that selection: {exc}")
            ready = False

    if not ready:
        st.info("👈 Select at least one Row and one Column for both the Numerator and Denominator.")
    else:
        m1, m2, m3 = st.columns(3)
        m1.metric("Numerator", f"{numerator_value:,.2f}")
        m2.metric("Denominator", f"{denominator_value:,.2f}")
        if denominator_value == 0:
            m3.metric("Result", "—")
            st.error("❌ Denominator is 0 for this selection — can't divide. Pick a different Denominator.")
        else:
            ratio = numerator_value / denominator_value
            m3.metric("Result (Num ÷ Denom)", f"{ratio:.2%}")
            st.caption(f"As a plain ratio: **{ratio:.4f}**")

            if st.button("➕ Add to Results Table", key=f"ratio_add_{key_suffix}"):
                st.session_state[table_key].append(
                    {
                        "Label": label_input.strip() or f"Ratio {len(st.session_state[table_key]) + 1}",
                        "Numerator (rows)": ", ".join(r.strip() for r in num_rows_sel),
                        "Numerator (cols)": ", ".join(num_cols_sel),
                        "Denominator (rows)": ", ".join(r.strip() for r in den_rows_sel),
                        "Denominator (cols)": ", ".join(den_cols_sel),
                        "Numerator Value": numerator_value,
                        "Denominator Value": denominator_value,
                        "Ratio %": ratio * 100,
                    }
                )
                st.success("Added below. Adjust the selections above to add another ratio.")

    saved_rows = st.session_state.get(table_key, [])
    if saved_rows:
        st.markdown("##### 📋 Saved Ratios")
        results_df = pd.DataFrame(saved_rows)
        try:
            styled_results = results_df.style.format(
                {"Numerator Value": "{:,.2f}", "Denominator Value": "{:,.2f}", "Ratio %": "{:,.2f}%"}
            )
            st.dataframe(styled_results, use_container_width=True)
        except Exception:
            st.dataframe(results_df, use_container_width=True)

        rcsv1, rcsv2 = st.columns(2)
        with rcsv1:
            ratio_csv_buffer = io.StringIO()
            results_df.to_csv(ratio_csv_buffer, index=False)
            st.download_button(
                "⬇️ Download Saved Ratios CSV",
                data=ratio_csv_buffer.getvalue(),
                file_name=f"{key_suffix}_ratios.csv",
                mime="text/csv",
                use_container_width=True,
                key=f"download_ratios_{key_suffix}",
            )
        with rcsv2:
            if st.button("🗑️ Clear Saved Ratios", key=f"ratio_clear_{key_suffix}", use_container_width=True):
    ...
    if not show_pivot_table:
        ...
        try:
            ...
            if pivot_source_df.empty:
                ...
                if pivot_col_cols:
                    st.markdown("#### 🎯 Stage Efficiency Table")
                    efficiency_display_df = render_stage_efficiency_table(
                        pivot_source_df,
                        pivot_row_cols,
                        pivot_col_cols,
                        pivot_value_col,
                    ...
                    efficiency_csv_buffer = io.StringIO()
                    efficiency_display_df.to_csv(efficiency_csv_buffer)
                    st.download_button(
                        "⬇️ Download Stage Efficiency Table CSV",
                        data=efficiency_csv_buffer.getvalue(),
                        file_name=f"{selected_table}_pivot_efficiency.csv",
                        mime="text/csv",
                        use_container_width=True,
                ...
                with dl_col2:
                    display_csv_buffer = io.StringIO()
                    display_pivot_df.to_csv(display_csv_buffer)
                    st.download_button(
                        "⬇️ Download Summary Matrix CSV (with subtotals)",
                        data=display_csv_buffer.getvalue(),
                ...
                # Runs only after the pivot table above is fully built — this is a
                # separate, later step, not part of the pivot process itself.
                render_pivot_ratio_calculator(display_pivot_df, key_suffix=selected_table)
        except SQLAlchemyError as exc:

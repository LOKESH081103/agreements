...
with st.sidebar:
    ...
    show_in_crores = st.checkbox(
        "Show sums in Crores (÷ 1,00,00,000)",
        value=False,
        key="show_in_crores",
        help=(
            "When on, all numeric totals (KPI cards, and the Monthly/Yearly "
            "sum columns below) are shown divided by 1,00,00,000 and suffixed "
            "'Cr'. Raw values are still kept in the downloadable CSVs."
        ),
    )
    ...
    if show_monthly_summary:
        ...
        if not date_columns:
            ...
            sum_columns: list[str] = st.multiselect(
                "Column(s) to SUM per month/year (optional) — e.g. emi",
                options=numeric_cols_for_sum,
                key=f"sum_columns_{selected_table}",
                help=(
                    "In addition to record counts, compute the SUM of these "
                    "numeric column(s) for every month and every year. Each "
                    "gets its own metric cards, chart, table, and CSV below."
                ),
            )

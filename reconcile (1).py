...
def find_better_neighbor(
    ...
    Returns (best_party_number, best_name_sim, best_addr_sim) or None if nothing
    meaningfully better is found.
    """
    rows = company_by_agreement.get(agreement_no)
    if rows is None or not name_norm:
        return None
    ...
    for _, crow in rows.iterrows():
        ...
        if abs(other_party - party_number) > search_radius:
            continue
        n_sim = similarity(name_norm, crow["_name_norm"] or "")
        if n_sim <= current_name_sim + 15:  # must be a meaningfully better name match
            continue
        a_sim = similarity(addr_norm, crow["_addr_norm"] or "")
        if best is None or n_sim > best[1]:
            best = (other_party, n_sim, a_sim)
...
def build_remarks(
    ...
    if status == "MATCH":
        parts.append(
            f"Name ({name_sim:.1f}%) and address ({addr_sim:.1f}%) both closely "
            "match the company record for this exact party number."
        )

    elif status == "PARTIAL MATCH":
        if name_sim < MATCH_THRESHOLD <= addr_sim:
            parts.append(
                f"Address matches well ({addr_sim:.1f}%), but the name is only "
                f"a partial match ({name_sim:.1f}%) — likely a shortened name, "
                "spelling variation, or OCR/extraction difference."
            )
        elif addr_sim < MATCH_THRESHOLD <= name_sim:
            parts.append(
                f"Name matches well ({name_sim:.1f}%), but the address is only "
                f"a partial match ({addr_sim:.1f}%) — likely abbreviations, "
                "reordering, or missing/extra address components."
            )
        else:
            parts.append(
                f"Both name ({name_sim:.1f}%) and address ({addr_sim:.1f}%) are "
                f"similar but fall short of the {MATCH_THRESHOLD}% match threshold "
                "— possible partial data, formatting differences, or typos."
            )
        ...
        if name_sim < PARTIAL_THRESHOLD and addr_sim < PARTIAL_THRESHOLD:
            parts.append(
                f"Both name ({name_sim:.1f}%) and address ({addr_sim:.1f}%) are "
                "significantly different from the company record at this party "
                "number — this looks like the wrong person entirely, not a "
                "formatting issue."
            )
        elif name_sim < PARTIAL_THRESHOLD:
            parts.append(
                f"Name is significantly different ({name_sim:.1f}%) even though "
                f"the address is closer ({addr_sim:.1f}%) — could be a different "
                "co-borrower living at the same/similar address, or a party-"
                "number misalignment (see hint below)."
            )
        elif addr_sim < PARTIAL_THRESHOLD:
            parts.append(
                f"Address is significantly different ({addr_sim:.1f}%) even "
                f"though the name is closer ({name_sim:.1f}%) — could be a "
                "stale/alternate address on one side, or the same person listed "
                "twice with different addresses in the company file."
            )
        else:
            parts.append(
                f"Name ({name_sim:.1f}%) and address ({addr_sim:.1f}%) are both "
                f"below the {PARTIAL_THRESHOLD}% threshold — review manually."
            )

    if neighbor_hint:
        other_party, n_sim, a_sim = neighbor_hint
        parts.append(
            f"NOTE: this extracted party matches company Party Number "
            f"{other_party} much better (Name: {n_sim:.1f}%, Address: "
            f"{a_sim:.1f}%) — likely a party-number offset/misalignment "
...
def reconcile(extracted: pd.DataFrame, company: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in merged.iterrows():
        ...
        remarks = build_remarks(status, name_sim, addr_sim, has_company_row, neighbor_hint)

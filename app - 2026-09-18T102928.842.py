...
def _month_sort_key(value: Any) -> tuple[int, int, str] | None:
    ...
    that orders by FINANCIAL YEAR cycle (April -> March), not calendar year
    (January -> December): April..December sort first within their calendar
    year, and January/February/March fold into the SAME financial year that
    started the previous calendar year (e.g. Apr-23...Dec-23, Jan-24, Feb-24,
    Mar-24 all land together in order, then Apr-24 starts the next cycle).
    Returns None for anything that doesn't look like a month value, so
    normal columns are completely unaffected.
    """
    ...
    if year_part:
        ...
        if year_num < 100:  # 2-digit year, e.g. "24" -> 2024
            year_num += 2000
        # Remap calendar month (Jan=1..Dec=12) to an April-first FY cycle
        # (Apr=1..Mar=12), folding Jan/Feb/Mar into the FY that started the
        # PREVIOUS calendar year.
        if month_num >= 4:
            fy_year, fy_month = year_num, month_num - 3
        else:
            fy_year, fy_month = year_num - 1, month_num + 9
    else:
        # No year at all -- every value is in the same single cycle, so
        # there's no "previous year" to fold Jan/Feb/Mar into. Just reorder
        # the months themselves to start at April.
        fy_year = 0
        fy_month = month_num - 3 if month_num >= 4 else month_num + 9

    return (fy_year, fy_month, text)
...
def _render_styled_pivot_matrix(
    ...
    """
    Render the Summary Matrix on screen.

    Deliberately NOT using .style.apply(fn, axis=None) here. That forces
    pandas to run an arbitrary Python function over the ENTIRE table, every
    single rerun, before a single row can be shown -- with a few thousand
    rows (a deep Rows hierarchy easily gets there) that eager, whole-table
    computation is the actual cause of the page freezing, not the database
    query or the pivot math itself (both already fast). Plain number
    formatting via .style.format() is cheap -- it's just a display format
    string, not a per-cell Python loop -- so that's kept; only the row/
    column background-color shading is dropped from this on-screen view.
    The full bold/shaded look is still there in the Excel download just
    below, which Excel renders natively on your machine, not this browser
    tab, so it doesn't have the same cost.
    """
    try:
        num_fmt = "{:,.0f}" if agg_func == "count" else "{:,.2f}"
        st.dataframe(display_df.style.format(num_fmt), use_container_width=True)
    except Exception:
        # If formatting fails for any reason (odd dtype, older pandas, etc.),
        # fall back to a plain, still-correct, unformatted table.
        st.dataframe(display_df, use_container_width=True)
    st.caption(
        "ℹ️ Subtotal-row and Total-column shading is kept out of this on-screen preview "
        "to stay fast on large tables — it's still there in the Excel download below."
    )
...
def dataframe_to_formatted_excel_bytes(
    ...
    if use_row_hierarchy_style or use_multiindex_hierarchy_style:
        ...
        # (e.g. "EAST" then "EAST_1"/"EAST_2" beneath it) -- summaryBelow=False
    ...
    # each row shares with the row directly above it. A row sharing 0 levels
    # with its predecessor starts a brand new top-level group (outline 0);
    # sharing 1 level starts a new second-level group nested under the same
    # top-level parent (outline 1); and so on. This is what lets Excel's
    # native +/- grouping work correctly even though this table has no
    # separate one-row-per-level subtotal rows to key off of -- every row is
    # already a full leaf combination, so the grouping is inferred purely
    # from how much of the row above's prefix carries over.
    multi_common_prefix: list[int] = []
    if use_multiindex_hierarchy_style:
        prev_tuple: tuple | None = None
        for r in range(n_rows):
            if is_grand_total_row(r):
                multi_common_prefix.append(0)
                prev_tuple = None  # never let anything match against the Total row
                continue
            tup = flat_index_tuples[r]
            common = 0
            if prev_tuple is not None:
                for lvl in range(n_index_cols):
                    if tup[lvl] == prev_tuple[lvl]:
                        common += 1
                    else:
                        break
            multi_common_prefix.append(common)
            prev_tuple = tup
    ...
    for r in range(n_rows):
        ...
        if use_row_hierarchy_style:
            ...
            ws.row_dimensions[excel_row].outlineLevel = depth
        elif use_multiindex_hierarchy_style:
            ws.row_dimensions[excel_row].outlineLevel = multi_common_prefix[r]
        ...
        for c in range(n_index_cols):
            ...
            if use_multiindex_hierarchy_style:
                ...
                cell.font = Font(name="Calibri", size=10.5, bold=is_total_row, color=level_color)
                if is_total_row:
                ...
                cell.font = Font(name="Calibri", size=10.5, bold=(is_total_row or is_group), color=level_color)
                if is_group and not is_total_row:
                    cell.fill = group_fill
                cell.alignment = Alignment(indent=depth, vertical="center")
...
def build_field_efficiency_pivot(
    ...
    # One shared cache across every numerator AND denominator lookup below --
    # see _make_dims_cache's docstring for why this is what keeps this table
    # fast even with a deep Rows hierarchy and many Columns values.
    series_for = _make_dims_cache(source_df, agg_func)

    def _eff_value(row_prefix: tuple[Any, ...], col_prefix: tuple[Any, ...]) -> float:
        """A (possibly partial, for a group row) row_prefix combined with a
        (possibly empty, for the Total column) col_prefix. Excluding
        field_normalize from BOTH the dims and the key for the denominator
        -- wherever it happens to live, row side or column side -- is what
        makes this one formula correct for every case: leaf cell, row-group
        subtotal, Total column, and the Total/Total corner alike."""
        numer_dims = tuple(field_row_cols[: len(row_prefix)]) + tuple(field_col_cols[: len(col_prefix)])
        numer_key = tuple(row_prefix) + tuple(col_prefix)
        numerator = _lookup_dims_value(series_for, numer_dims, numer_key)

        if field_normalize in numer_dims:
            idx = numer_dims.index(field_normalize)
            denom_dims = numer_dims[:idx] + numer_dims[idx + 1 :]
            denom_key = numer_key[:idx] + numer_key[idx + 1 :]
        else:
            denom_dims, denom_key = numer_dims, numer_key
        denominator = _lookup_dims_value(series_for, denom_dims, denom_key)
    ...
    for re_ in row_entries:
        ...
        row_vals = [_eff_value(re_["prefix"], cp) for cp in col_prefixes]
        row_vals.append(_eff_value(re_["prefix"], ()))  # Total column: row prefix as-is, no column constraint
        data_rows.append(row_vals)
...
def _render_styled_efficiency_matrix(
    ...
    """
    Render the Field Efficiency Table on screen. Same reasoning as
    `_render_styled_pivot_matrix`: .style.apply(fn, axis=None) forces an
    eager, whole-table Python computation on every rerun -- the actual
    freeze cause on a large table -- so the on-screen view now only does
    cheap number-format-as-% via .style.format(), and the full bold/shaded
    Total-row/column/group treatment lives in the Excel download instead.
    """
    try:
        st.dataframe(pivot_eff.style.format("{:,.2f}%"), use_container_width=True)
    except Exception:
        st.dataframe(pivot_eff, use_container_width=True)
    st.caption(
        "ℹ️ Total-row/column (and group-row) shading is kept out of this on-screen preview "
        "to stay fast on large tables — it's still there in the Excel download below."
    )
...
def _compute_flow_retention_from_rows(display_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, label in enumerate(display_df.index):
        if label == "Grand Total":
            continue
        # Use positional (.iloc) access, not .loc[label] -- child rows only
        # ever show their own value (e.g. "↳ Flow"), never the parent slab's
        # name, so the SAME label text appears once per group. .loc[label]
        # would ambiguously return every group's "Flow" row at once.
        ...
        if current_group is None:
            continue
        stripped = label.split("↳", 1)[-1].strip()
        key = _flow_alias_key(stripped)
        if key:
            groups[current_group][key] = row

    zeros = pd.Series(0.0, index=display_df.columns)
    result_rows: dict[str, pd.Series] = {}
    for group in group_order:
        parts = groups[group]
        flow = parts.get("flow")
        if flow is None:
            continue  # no Flow sub-row -> metric doesn't apply to this group
        normalised = parts.get("normalised", zeros)
        stabilized = parts.get("stabilized", zeros)
        rollback = parts.get("rollback", zeros)
        total = flow + normalised + stabilized + rollback
        pct_flow = (flow / total.replace(0, pd.NA)) * 100
        result_rows[group] = (100 - pct_flow).fillna(0.0)

    if not result_rows:
        return pd.DataFrame(columns=display_df.columns)
    return pd.DataFrame(
        [result_rows[g] for g in result_rows], index=list(result_rows.keys()), columns=display_df.columns
    )
...
def render_field_efficiency_table(
    ...
    (display_df, group_row_flags) for CSV/Excel export."""
    ...
    return pivot_eff, group_row_flags
...
def render_multi_value_field_efficiency_table(
    ...
    concatenating, same safeguard as the Summary Matrix version -- so the
    FIRST block's group_row_flags (the row tree/order every other block was
    reindexed to match) is what's returned for CSV/Excel export.
    """
    blocks: dict[str, pd.DataFrame] = {}
    total_col_flags: list[bool] = []
    base_index: pd.Index | None = None
    ...
    return pivot_eff, base_group_row_flags
                ...
                if pivot_render_allowed:
                    ...
                    if use_flow_retention:
                        ...
                        if efficiency_display_df.empty:
                            ...
                            st.dataframe(efficiency_display_df.style.format("{:,.2f}%"), use_container_width=True)
                    ...
                    if use_flow_retention:
                        ...
                        if len(pivot_value_cols) == 1:
                            efficiency_display_df, eff_group_row_flags = render_field_efficiency_table(
                                pivot_source_df,
                                pivot_row_cols,
                                pivot_col_cols,
                                normalize_field,
                            ...
                            efficiency_display_df, eff_group_row_flags = render_multi_value_field_efficiency_table(
                                pivot_source_dfs,
                                pivot_row_cols,
                                pivot_col_cols,
                                normalize_field,

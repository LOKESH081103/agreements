...
# --------------------------------------------------------------------------- #
# Shared DB/pivot engine, imported from db_core.py
# --------------------------------------------------------------------------- #
# `db_core.py` holds the framework-agnostic half of this app (schema
# introspection, query building, the Excel-style pivot/DPD-bucket/Field-
# Efficiency engine, Excel export, and auth helpers) so that this Streamlit
# app and the FastAPI service (`api.py`) always compute identical numbers
# from ONE source of truth instead of two copies that can silently drift
# apart. Every name below was diffed byte-for-byte against this file's
# previous, independently-tested implementation before being deleted here --
# nothing changed except where the code now physically lives.
#
# `db_core.get_engine()` is intentionally NOT imported/used here: it only
# reads plain environment variables, while this app also supports
# `st.secrets` (for Streamlit Cloud deployments) via `_get_config_value()`
# above -- that's a genuine Streamlit-specific behavior difference, not
# duplication, so `get_engine`/`get_engine_safe` stay defined locally.
import db_core as _core

# Pure functions / constants -- no caching needed, used exactly as-is.
classify_column = _core.classify_column
build_query = _core.build_query
_combine_group_stats = _core._combine_group_stats
_expand_date_fields = _core._expand_date_fields
_make_dims_cache = _core._make_dims_cache
_lookup_dims_value = _core._lookup_dims_value
_month_sort_key = _core._month_sort_key
_ordered_unique = _core._ordered_unique
_reorder_axis_chronologically = _core._reorder_axis_chronologically
_build_row_entries = _core._build_row_entries
_build_col_entries = _core._build_col_entries
_pretty_prefix = _core._pretty_prefix
_split_matrix_row_label = _core._split_matrix_row_label
_effective_col_level = _core._effective_col_level
add_dpd_buckets_to_excel_pivot = _core.add_dpd_buckets_to_excel_pivot
_date_field_leaf = _core._date_field_leaf
_prettify_date_leaf_axes = _core._prettify_date_leaf_axes
_clean_efficiency_column_label = _core._clean_efficiency_column_label
slice_field_efficiency_to_status = _core.slice_field_efficiency_to_status
build_pivot_table = _core.build_pivot_table
MONTH_NAMES = _core.MONTH_NAMES
CRORE = _core.CRORE
_is_employee_authorized = _core._is_employee_authorized
_get_password_hash = _core._get_password_hash
_account_exists = _core._account_exists
_create_account = _core._create_account
_verify_password = _core._verify_password
_touch_last_login = _core._touch_last_login
_log_audit_event = _core._log_audit_event
_too_many_recent_failures = _core._too_many_recent_failures

# Functions that were `@st.cache_data`/`@st.cache_resource`-wrapped here --
# re-apply the exact same decorator/ttl on top of db_core's plain version,
# so caching behavior is unchanged even though the logic now lives there.
list_tables = st.cache_data(ttl=300, show_spinner=False)(_core.list_tables)
get_columns = st.cache_data(ttl=300, show_spinner=False)(_core.get_columns)
get_distinct_values = st.cache_data(ttl=300, show_spinner=False)(_core.get_distinct_values)
get_distinct_values_filtered = st.cache_data(ttl=300, show_spinner=False)(_core.get_distinct_values_filtered)
get_available_years = st.cache_data(ttl=300, show_spinner=False)(_core.get_available_years)
get_available_year_months = st.cache_data(ttl=300, show_spinner=False)(_core.get_available_year_months)
run_query = st.cache_data(ttl=60, show_spinner=False)(_core.run_query)
fetch_pivot_source_data = st.cache_data(ttl=600, show_spinner=False)(_core.fetch_pivot_source_data)
build_excel_style_pivot = st.cache_data(ttl=300, show_spinner=False)(_core.build_excel_style_pivot)
dataframe_to_formatted_excel_bytes = st.cache_data(ttl=300, show_spinner=False)(_core.dataframe_to_formatted_excel_bytes)
build_field_efficiency_pivot = st.cache_data(ttl=300, show_spinner=False)(_core.build_field_efficiency_pivot)
ensure_auth_tables = st.cache_resource(show_spinner=False)(_core.ensure_auth_tables)
...
PIVOT_HIERARCHY_FIELDS = ("zone", "sub_zone", "region", "sub_region", "area", "branch")


def read_master_hierarchy(uploaded_file) -> pd.DataFrame:
    """Read a hierarchy template from Excel, CSV, or delimiter-separated TXT."""
    filename = uploaded_file.name.lower()
    uploaded_file.seek(0)
    if filename.endswith((".xlsx", ".xlsm", ".xls", ".xlsb")):
        master = pd.read_excel(uploaded_file, sheet_name=0, engine=_excel_engine_for(filename))
    else:
        # sep=None asks Python's CSV sniffer to distinguish common comma/tab files.
        try:
            master = pd.read_csv(uploaded_file, sep=None, engine="python", encoding="utf-8-sig")
        except (pd.errors.ParserError, UnicodeDecodeError):
            uploaded_file.seek(0)
            first_line = uploaded_file.readline().decode("utf-8-sig", errors="replace")
            delimiter = "\t" if first_line.count("\t") > first_line.count(",") else ","
            uploaded_file.seek(0)
            master = pd.read_csv(uploaded_file, sep=delimiter, encoding="utf-8-sig")

    # Match common header variants such as "Sub Zone", "sub_zone", and "SUBZONE".
    canonical_to_field = {re.sub(r"[^a-z0-9]", "", field.lower()): field for field in PIVOT_HIERARCHY_FIELDS}
    selected_columns: dict[str, Any] = {}
    for column in master.columns:
        canonical = re.sub(r"[^a-z0-9]", "", str(column).strip().lower())
        field = canonical_to_field.get(canonical)
        if field and field not in selected_columns:
            selected_columns[field] = column

    missing = [field for field in PIVOT_HIERARCHY_FIELDS if field not in selected_columns]
    if missing:
        raise ValueError(
            "Master file is missing hierarchy columns: " + ", ".join(missing) +
            ". Expected zone, subzone, region, subregion, area, and branch."
        )

    result = master[[selected_columns[field] for field in PIVOT_HIERARCHY_FIELDS]].copy()
    result.columns = list(PIVOT_HIERARCHY_FIELDS)
    result = result.dropna(how="all").drop_duplicates().reset_index(drop=True)
    if result.empty:
        raise ValueError("The master hierarchy file contains no hierarchy rows.")
    return result


def _hierarchy_match_key(value: Any) -> str | None:
    """Normalize join keys to tolerate whitespace and case differences."""
    if pd.isna(value):
        return None
...
def add_missing_master_pivot_rows(
    source_df: pd.DataFrame,
    ...
    """Append zero-stat rows for master row/column combinations absent in the query."""
    compact_to_master = {re.sub(r"[^a-z0-9]", "", c.lower()): c for c in master_df.columns}
    row_master_columns: dict[str, str] = {}
    for field in row_cols:
        master_field = compact_to_master.get(re.sub(r"[^a-z0-9]", "", field.lower()))
        if master_field is None or field not in source_df.columns:
            return source_df, 0, (
                f"Cannot apply the master grid: Rows field `{field}` is not a matching hierarchy column."
            )
        row_master_columns[field] = master_field

    master_rows = pd.DataFrame({field: master_df[master_field] for field, master_field in row_master_columns.items()})
    # Preserve active sidebar filters when they target fields represented in the master.
    for filter_field, selected_values in filter_values.items():
        if not selected_values:
            continue
        master_field = compact_to_master.get(re.sub(r"[^a-z0-9]", "", filter_field.lower()))
        if master_field is not None:
            allowed = {_hierarchy_match_key(value) for value in selected_values}
            mask = master_df[master_field].map(_hierarchy_match_key).isin(allowed)
            master_rows = master_rows.loc[mask].copy()

    master_rows = master_rows.dropna(subset=row_cols).drop_duplicates().reset_index(drop=True)
    if master_rows.empty:
        return source_df, 0, "No master hierarchy rows match the selected filters."

    effective_col_cols = [
        expanded
        for field in col_cols
        for expanded in (
            [f"__{field}__month", f"__{field}__date"]
            if column_kind_map.get(field) == "date" else [field]
        )
    ]

    grid = master_rows
    if col_cols:
        if any(field not in source_df.columns for field in effective_col_cols):
            return source_df, 0, "Cannot apply the master grid because a selected Columns field is unavailable."
        observed_columns = source_df[effective_col_cols].drop_duplicates()
        if observed_columns.empty:
            # A filtered query may return no transactions at all. Keep the
            # master rows visible under one explicit empty-category heading.
            observed_columns = pd.DataFrame(
                [{field: "(no data)" for field in effective_col_cols}],
                columns=effective_col_cols,
            )
        grid = grid.assign(__master_grid_join__=1).merge(
            observed_columns.assign(__master_grid_join__=1), on="__master_grid_join__", how="inner"
        ).drop(columns="__master_grid_join__")

    join_cols = [*row_cols, *effective_col_cols]
    existing_keys = {
        tuple(_hierarchy_match_key(value) for value in row)
        for row in source_df[join_cols].itertuples(index=False, name=None)
    }
    grid_keys = [
        tuple(_hierarchy_match_key(value) for value in row)
        for row in grid[join_cols].itertuples(index=False, name=None)
    ]
    missing_mask = [key not in existing_keys for key in grid_keys]
    missing_rows = grid.loc[missing_mask].copy()
    if missing_rows.empty:
        return source_df, 0, None

    missing_rows["__stat_sum__"] = 0.0
    missing_rows["__stat_count__"] = 0.0
    # NaN min/max values do not distort active ancestor totals; the rendered
    # leaf cells are filled to zero after pivot construction below.
    missing_rows["__stat_min__"] = np.nan
    missing_rows["__stat_max__"] = np.nan
    return pd.concat([source_df, missing_rows[source_df.columns]], ignore_index=True), len(missing_rows), None
...
with st.sidebar:
    ...
    include_all_master_rows = False
    master_hierarchy_df: pd.DataFrame | None = None

    if show_pivot_table:
        include_all_master_rows = st.checkbox(
            "Include all master hierarchy rows from .txt file (display missing rows as 0)?",
            value=False,
            key=f"include_master_hierarchy_{selected_table}",
            help="Upload a .txt, .csv, or Excel hierarchy file with zone, subzone, region, subregion, area, and branch columns.",
        )
        master_hierarchy_file = st.file_uploader(
            "Master hierarchy file (.txt / .csv / Excel)",
            type=["txt", "csv", "xlsx", "xlsm", "xls", "xlsb"],
            key=f"master_hierarchy_file_{selected_table}",
            disabled=not include_all_master_rows,
        )
        if include_all_master_rows:
            if master_hierarchy_file is None:
                st.info("Upload the master hierarchy file to include missing rows.")
            else:
                try:
                    master_hierarchy_df = read_master_hierarchy(master_hierarchy_file)
                    st.caption(f"Loaded **{len(master_hierarchy_df):,}** unique master hierarchy rows.")
                except Exception as exc:  # noqa: BLE001
                    st.error(f"Could not read the master hierarchy file: {exc}")
...
with tab_pivot:
    ...
    if not show_pivot_table:
        ...
        try:
            ...
            master_rows_added = 0
            if include_all_master_rows and master_hierarchy_df is not None:
                merge_messages: list[str] = []
                for value_col, source_df in list(pivot_source_dfs.items()):
                    completed_df, added_count, merge_message = add_missing_master_pivot_rows(
                        source_df,
                        master_hierarchy_df,
                        pivot_row_cols,
                        pivot_col_cols,
                    ...
                    pivot_source_dfs[value_col] = completed_df
                    master_rows_added = max(master_rows_added, added_count)
                    if merge_message:
                        merge_messages.append(merge_message)
                for message in dict.fromkeys(merge_messages):
                    st.warning(message)
            ...
            if any(df.empty for df in pivot_source_dfs.values()):
                ...
                if master_rows_added:
                    st.info(f"Added **{master_rows_added:,}** master hierarchy row/column combination(s) with zero values.")
                st.caption(
                ...
                if pivot_render_allowed:
                    ...
                    if include_all_master_rows and master_hierarchy_df is not None:
                        # Empty combinations use NaN for min/max statistics so
                        # they cannot skew parent totals; present those leaf
                        # cells as zero only after the real aggregates are built.
                        display_pivot_df = display_pivot_df.fillna(0)

...
def display_virtualized_pivot(
    ...
    **kwargs
    ...
    # Blazing-fast vectorized numeric extraction via NumPy
    if data_indices:
        raw_vals = df.iloc[:, data_indices].to_numpy()
        # Clean inf and nan in bulk via numpy
        if np.issubdtype(raw_vals.dtype, np.number):
            mask = ~np.isfinite(raw_vals)
            if mask.any():
                raw_vals = np.where(mask, np.nan, raw_vals)
        for idx_num, fkey in enumerate(field_keys):
            flat_data[fkey] = raw_vals[:, idx_num].tolist()

...
def _render_aggrid_matrix(
    ...
    AgGrid instead of a fully-rendered static HTML table. AgGrid only ever
    mounts DOM nodes for the rows currently scrolled into view, so a table
    with tens of thousands of rows no longer freezes the browser the way a
    fully-rendered `st.dataframe`/HTML table could.

    Design notes (deliberate, not oversights):
    - The Rows hierarchy stays exactly what `build_excel_style_pivot`
      already produces: ONE pinned "Row Labels" text column, indented per
      level (using non-breaking spaces so the browser doesn't collapse the
      indentation) -- not one AgGrid column per hierarchy level. AG Grid's
      *Row Grouping / Tree Data* mode (which is what "one column per
      level" really implies) is an AG Grid ENTERPRISE feature requiring a
      separate license key; this keeps the exact same tree already
      computed, just rendered virtualized, with zero licensing dependency.
    - Bold + light-blue shading on subtotal/group rows and light-grey
      shading on Total columns reuse the exact same colors as the Excel
      export (`_XL_GROUP_ROW_FILL` / `_XL_TOTAL_COL_FILL`) -- previously
      dropped from the on-screen view specifically because a whole-table
      `.style.apply()` was too slow; AgGrid applies these per-row/per-cell
      without that cost, so the shading is restored on screen too.
...
def _render_styled_pivot_matrix(
    ...
    `_render_aggrid_matrix`). Only the rows scrolled into view are ever
    ...
    try:
        _render_aggrid_matrix(display_df, group_row_flags, total_col_flags, num_fmt)
    except Exception:
...
def _render_styled_efficiency_matrix(
    ...
    (see `_render_aggrid_matrix`) -- same reasoning as
    ...
    """
    try:
        _render_aggrid_matrix(pivot_eff, group_row_flags or [], total_col_flags, "{:,.2f}%")
    except Exception:

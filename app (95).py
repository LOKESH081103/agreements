...
with st.sidebar:
    ...
    st.divider()
    st.subheader("4️⃣ Excel-Style Pivot Table (optional)")
    show_pivot_table = st.checkbox(
        "Show monthly cumulative summary / Pivot Table",
        value=False,
    ...
    if show_pivot_table:
        ...
        pivot_run_clicked = st.button(
            "🧮 Build Pivot Table", type="primary", use_container_width=True, key=f"pivot_run_{selected_table}"
        )
...
tab_report, tab_pivot, tab_sql = st.tabs(
    ["📊 Dynamic Report Builder", "🧮 Pivot Table", "🧑‍💻 Custom SQL Query"]
)
...
with tab_pivot:
    st.subheader("🧮 Excel-Style 4-Quadrant Pivot Table")

...
def render_labeled_bar_chart(
    ...
    try:
        png_bytes = fig.to_image(format="png", scale=res_scale)
        st.download_button(
            "⬇️ Download Chart as High-Resolution PNG",
            data=png_bytes,
            file_name=f"{filename_prefix}.png",
            mime="image/png",
            key=f"download_png_{key_suffix}",
        )
    except Exception:
        # kaleido isn't installed / failed — the toolbar camera icon still works.
        st.caption(
            "💡 Tip: click the camera icon in the chart's toolbar to export a "
            f"high-resolution PNG at ≈{1600 * res_scale}×{800 * res_scale}px."
        )
...
with tab_report:
    ...
    if show_monthly_summary and summary_date_col:
        ...
        yearly_df = summary_df.groupby(group_cols, as_index=False)["record_count"].sum()
        ...
        if focus_years_summary:
            yearly_display_df = yearly_display_df[yearly_display_df["yr"].isin(focus_years_summary)]
        if breakdown_col and focus_categories:
            yearly_display_df = yearly_display_df[yearly_display_df[breakdown_col].isin(focus_categories)]

        if yearly_display_df.empty:
            st.warning("No records match the selected year/category focus. Try widening your focus selections.")
        elif breakdown_col:
            render_labeled_bar_chart(
                yearly_display_df,
                x_col="year_label",
